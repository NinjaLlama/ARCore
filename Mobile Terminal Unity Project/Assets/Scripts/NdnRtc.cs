...
//		uint uvPLaneSize = yPlaneSize / 2;
		...
		GCHandle pinnedBuffer = GCHandle.Alloc (pixelBuffer, GCHandleType.Pinned);
...
//		public static extern int ndnrtc_LocalVideoStream_incomingARGBFrame (IntPtr stream,
//			uint width, uint height, IntPtr argbFrameData, uint frameSize);
		int frameNo = NdnRtcWrapper.ndnrtc_LocalVideoStream_incomingARGBFrame (ndnrtcHandle_, (uint)width, (uint)height, buffer, (uint)bufferSize);
